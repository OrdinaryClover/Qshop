...
class MiddleWareTest(MiddlewareMixin):
    ...
    def process_view(self,request,callback,callbackargs,callbackkwargs):
        """
        请求经过process_request 之后被触发的方法
        :param request:   request是包含请求信息的请求对象，但是有可能被process_request处理
        :param callback:   处理请求的视图
        :param callbackargs:   参数  元组
    ...
    def process_template_response(self,request,response):
        ...
        :return:
        """
        # print("process_templates_response")
        return response
    def process_response(self,request,response):
        """
        返回响应    可以对所有的返回结果做处理   例如：设置返回的响应头  设置数据类型 设置cookie session 版本号 返回统一的格式
        :param request:
        :param response:
        :return:
        """
